...
public class SearchInputGUI extends BaseGUI {
    ...
    public SearchInputGUI(supershop plugin, Player player, String currentSearch) {
        super(plugin, player, "&6Search Input", 54);
    ...
    private void setupKeyboard() {
        // First row: Q W E R T Y U I O (slots 10-18)
        int slot = 10;
        ...
        // Second row: A S D F G H J K L (slots 19-27)
        slot = 19;
        ...
        // Third row: Z X C V B N M (slots 28-34)
        slot = 28;
        for (int i = 0; i < 7; i++) {
            char c = KEYBOARD[2][i];
            ItemStack key = createButton(Material.WHITE_CONCRETE, "&f" + Character.toUpperCase(c),
        ...
        inventory.setItem(35, spaceBar);
        inventory.setItem(36, spaceBar);
    }
    ...
    protected void handleClick(InventoryClickEvent event) {
        ...
        if (slot >= 10 && slot <= 18) {
            ...
            // Second row
            char c = KEYBOARD[1][slot - 19];
            addCharacter(c);
        } else if (slot >= 28 && slot <= 34) {
            // Third row
            char c = KEYBOARD[2][slot - 28];
            addCharacter(c);
