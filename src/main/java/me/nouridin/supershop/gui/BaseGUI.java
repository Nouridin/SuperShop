...
public abstract class BaseGUI {
    ...
    public BaseGUI(supershop plugin, Player player, String title, int size) {
        this.plugin = plugin;
        this.player = player;
        this.title = plugin.getMessageUtils().colorize(title);
        this.size = size;
    ...
    protected ItemStack createButton(Material material, String name, String... lore) {
        ...
        if (meta != null) {
            meta.displayName(Component.text(plugin.getMessageUtils().colorize(name)));
            if (lore.length > 0) {
                List<Component> componentLore = Arrays.stream(lore)
                    .map(line -> Component.text(plugin.getMessageUtils().colorize(line)))
