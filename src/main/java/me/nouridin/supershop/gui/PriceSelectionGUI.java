...
public class PriceSelectionGUI extends BaseGUI {
    ...
    public PriceSelectionGUI(supershop plugin, Player player, AddItemGUI parentGUI) {
        super(plugin, player, plugin.getLocaleManager().getMessage(player.getUniqueId(), "gui.priceselection.title"), 54);
    ...
    private void setupInstructions() {
        ...
        ItemStack instructions = createInfoItem(plugin.getLocaleManager().getMessage(playerUUID, "gui.priceselection.instructions.title"),
                plugin.getLocaleManager().getMessage(playerUUID, "gui.priceselection.instructions.lore1"),
                plugin.getLocaleManager().getMessage(playerUUID, "gui.priceselection.instructions.lore2"),
                plugin.getLocaleManager().getMessage(playerUUID, "gui.priceselection.instructions.lore3"));
    ...
    private void setupMaterialGrid() {
        ...
        for (int i = startIndex; i < CURRENCY_MATERIALS.length && itemsShown < itemsPerPage; i++) {
            ...
            ItemStack button = createButton(material, "&e" + getDisplayName(material),
                    plugin.getLocaleManager().getMessage(playerUUID, "gui.priceselection.material.lore1"),
                    plugin.getLocaleManager().getMessage(playerUUID, "gui.priceselection.material.lore2", String.valueOf(getCurrentQuantity(material))));
    ...
    private void setupCurrentPriceItems() {
        ...
        ItemStack priceLabel = createInfoItem(plugin.getLocaleManager().getMessage(playerUUID, "gui.priceselection.current-price.title"),
                plugin.getLocaleManager().getMessage(playerUUID, "gui.priceselection.current-price.lore"));
        ...
        for (int i = 0; i < Math.min(priceItems.size(), 6); i++) {
            ...
            ItemStack button = createButton(priceItem.getMaterial(),
                    plugin.getLocaleManager().getMessage(playerUUID, "gui.priceselection.current-price-item.name", String.valueOf(priceItem.getQuantity()), getDisplayName(priceItem.getMaterial())),
                    plugin.getLocaleManager().getMessage(playerUUID, "gui.priceselection.current-price-item.lore1"),
                    plugin.getLocaleManager().getMessage(playerUUID, "gui.priceselection.current-price-item.lore2"),
                    plugin.getLocaleManager().getMessage(playerUUID, "gui.priceselection.current-price-item.lore3"));
    ...
    private void setupControls() {
        ...
        // Done button
        ItemStack done = createButton(Material.EMERALD, plugin.getLocaleManager().getMessage(playerUUID, "gui.done"),
                plugin.getLocaleManager().getMessage(playerUUID, "gui.priceselection.done-button.lore"));
